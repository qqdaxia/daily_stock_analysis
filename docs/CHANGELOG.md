...
- [新功能] A 股大盘复盘推送新增“热门板块 / 热门股票”附加摘要，按板块涨跌幅 Top N 与个股涨跌幅优先、成交额次排序追加展示；任一数据源缺失时仅跳过对应区块，不阻断原复盘输出；本次仅复用既有 `DataFetcherManager` / TickFlow / efinance / AkShare fallback 与复盘拼装链路，不涉及模型 provider / Base URL / `.env` 配置迁移
